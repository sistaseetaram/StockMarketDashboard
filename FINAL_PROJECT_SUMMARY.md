# Final Project Objective and Summary
...
## Summary
The application is a real-time stock market dashboard built using a multi-page architecture with the **Streamlit** framework.

### Architecture and Tech Stack
- **Frontend & Routing**: Managed via `app.py` and a custom multi-page framework (`multiapp.py`). The project uses Streamlit version 1.8.1. Pages include Home, Finance Dashboard, and Prediction, located in the `apps/` directory.
- **Data Acquisition**: Utilizes the `yfinance` API for real-time and historical stock data.
- **Web Scraping**: Extracts top market movers (gainers, losers, active) and financial news from external sources like Yahoo Finance and Google News using `BeautifulSoup`.
